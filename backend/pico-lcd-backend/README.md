...
```bash
pico-lcd-backend$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
pico-lcd-backend$ sam local start-api
pico-lcd-backend$ curl http://localhost:3000/
```
...
```bash
pico-lcd-backend$ sam logs -n HelloWorldFunction --stack-name pico-lcd-backend --tail
```
...
```bash
pico-lcd-backend$ cd hello-world
...
```bash
sam delete --stack-name pico-lcd-backend
```
